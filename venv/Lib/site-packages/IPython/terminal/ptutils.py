...
def _elide_point(string: str, *, min_elide=30) -> str:
    ...
    if len(object_parts) > 3:
        return '{}.{}\N{HORIZONTAL ELLIPSIS}{}.{}'.format(object_parts[0], object_parts[1][0], object_parts[-2][-1],
                                                          object_parts[-1])

    elif len(file_parts) > 3:
        return ('{}' + os.sep + '{}\N{HORIZONTAL ELLIPSIS}{}' + os.sep + '{}').format(file_parts[0], file_parts[1][0],
                                                                                      file_parts[-2][-1],
                                                                                      file_parts[-1])
...
class IPythonPTCompleter(Completer):
    ...
    def _get_completions(body, offset, cursor_position, ipyc):
        ...
        for c in completions:
            ...
            if c.type == 'function':
                yield Completion(adjusted_text, start_position=c.start - offset,
                                 display=_elide(display_text + '()', body[c.start:c.end]),
                                 display_meta=c.type + c.signature)
            else:
                yield Completion(adjusted_text, start_position=c.start - offset,
                                 display=_elide(display_text, body[c.start:c.end]), display_meta=c.type)

...
class WidgetRegistry(object):
    ...
    def register(self, model_module, model_module_version_range, model_name, view_module, view_module_version_range,
                 view_name, klass):
        """Register a value"""
                    ...
                    for view_module, vv in sorted(vm.items()):
                        for view_version, vn in sorted(vv.items()):
                            for view_name, widget in sorted(vn.items()):
                                yield (model_module, model_version, model_name, view_module, view_version,
                                       view_name), widget
...
class Widget(LoggingHasTraits):
    ...
    def handle_comm_opened(comm, msg):
        ...
        if version.split('.')[0] != PROTOCOL_VERSION_MAJOR:
            raise ValueError("Incompatible widget protocol versions: received version %r, expected version %r" % (
    ...
    _view_module_version = Unicode('',
                                   help="A semver requirement for the namespace version containing the view.").tag(
        sync=True)

    _view_count = Int(None, allow_none=True,
                      help="EXPERIMENTAL: The number of views of the model displayed in the frontend. This attribute is experimental and may change or be removed in the future. None signifies that views will not be tracked. Set this to 0 to start tracking view creation/deletion.").tag(
        sync=True)
